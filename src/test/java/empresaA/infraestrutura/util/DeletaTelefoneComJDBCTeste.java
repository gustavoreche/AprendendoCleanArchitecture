...
public class DeletaTelefoneComJDBCTeste {
	...
	public void executa_tudoValido_retornoOk() {
		Assert.assertTrue(new DeletaTelefoneComJDBC(conexao).executa(new FuncionarioBuilder("111.111.111-18", "Gustavo").cria()));
	}
	...
	public void executa_semConexao_retornoException() {
		Assert.assertThrows(DeletaTelefoneException.class, () -> {
			Assert.assertTrue(new DeletaTelefoneComJDBC(null).executa(new FuncionarioBuilder("111.111.111-18", "Gustavo").cria()));
		});
	}
